...
public final class AmazonKinesisApplicationSample {
	...
	private static EndpointConfiguration endpointConfig;
	...
	private static void init() {
		...
		endpointConfig = new EndpointConfiguration("http://localhost:4566", Regions.US_EAST_1.getName());
		...
		kinesis = AmazonKinesisClientBuilder.standard().withCredentials(credentialsProvider)
		...
		cloudWatchClient = AmazonCloudWatchClientBuilder.standard().withCredentials(credentialsProvider)
				.withEndpointConfiguration(endpointConfig).build();
	}
	...
	public static void main(String[] args) throws Exception {
		...
		Worker worker = new Worker.Builder().config(kinesisClientLibConfiguration)
				.recordProcessorFactory(recordProcessorFactory).kinesisClient(kinesis)
				.dynamoDBClient(dynamoDB).cloudWatchClient(cloudWatchClient).build();
